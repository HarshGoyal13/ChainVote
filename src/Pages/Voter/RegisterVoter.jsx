...
const addDataInDB = async () => {
  ...
  try {
    const res = await axios.post("https://chainvote.onrender.com/voting/voter/addVoter", data);
    console.log(res);
  ...
  try {
    ...
    const handelRegisterVoter = async(e)=>{
      ...
      try{
        ...
        await contractInstance.registerVoter(formData.name,formData.image, ageNumber, formData.gender);
        ...
        setFormData({ name: "", age: "", gender: "", image: "",});
        navigate("/voterList");
        setMessage("");
        setLoading(false)
        ...
        console.log(error);
        toast.error('Something went wrong in Voter Registration');
        setLoading(false)
      }
    }
  ...
  <section className="px-5 lg:px-0">
    ...
    <div className="w-full max-w-[600px] mt-[30px] mx-auto bg-gradient-to-br from-gray-900 to-gray-800 rounded-xl shadow-xl p-10">
      <h3 className="text-white text-[28px] font-extrabold text-center mb-8">
        Register As, <span className="text-green-500">Voter</span>!
      </h3>
